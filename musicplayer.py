...
root.configure(background='#333333') #configure to set various attributes
root.resizable(False, False) #two argumnets frst for height and den 2nd for width to stop reesizing of window
mixer.init()
...
def PlayMusic():
    ...
    mixer.music.load(Playlist.get(ACTIVE)) #The pygame.mixer module is used to load, play, and manipulate audio files within a Pygame application. 
    mixer.music.play()
    
    
# icon
lower_frame = Frame(root , bg = "#FFFFFF", width = 485 , height = 180 )
lower_frame.place ( x = 0 , y = 400) #allows to place the window
...
# Label     --- used to display widget objects  
...
Button(root, text="Find Songs", width=59, height=1, font=("calibri",
